...
test.describe("Bloque 0 — el email no es público", () => {
    ...
    test("el rol anónimo no puede seleccionar profiles", async () => {
        const supabase = createClient(
            process.env.NEXT_PUBLIC_SUPABASE_URL!,
            process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
        );
        const { error } = await supabase.from("profiles").select("email").limit(1);
        // La Data API no debe exponer ninguna fila operativa al rol anónimo.
        expect(error).not.toBeNull();
    });

    test("el rol anónimo tampoco puede leer los campos antes públicos", async () => {
        const supabase = createClient(
            process.env.NEXT_PUBLIC_SUPABASE_URL!,
            process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
        );
