-- Check the dependency OID stored in pg_policy rather than trusting deparsed SQL
-- text, which can hide or change schema qualification under a hardened search_path.
...
with app_tables(name) as (
  ...
  select coalesce(bool_and(exists (
    ...
    where ns.nspname = 'public'
      and rel.relname = a.name
      and pol.polname = 'community access required'
      and pol.polpermissive = false
      and pol.polcmd = '*'
      and pol.polroles = array[(select oid from pg_roles where rolname = 'authenticated')]
      and pol.polqual::text ~ format(':funcid %s([^0-9]|$)', 'private.has_community_access()'::regprocedure::oid)
      and pol.polwithcheck::text ~ format(':funcid %s([^0-9]|$)', 'private.has_community_access()'::regprocedure::oid)
