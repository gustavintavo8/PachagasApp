-- Keep the RLS contract aligned with the policy invariant it is meant to prove:
-- restrictive, FOR ALL, authenticated-only policies that gate both reads and writes.
...
with app_tables(name) as (
  ...
  select coalesce(bool_and(exists (
    ...
    where p.schemaname = 'public'
      ...
      and coalesce(p.qual, '') ~ '(^|[^[:alnum:]_])(private\.)?has_community_access\(\)'
      and coalesce(p.with_check, '') ~ '(^|[^[:alnum:]_])(private\.)?has_community_access\(\)'
